...
public class ChunkContainer {
	...
	public Chunk getChunk(Vector3 pos) {
		return getChunk((int) Math.floor(pos.x / Chunk.WIDTH), (int) Math.floor(pos.z / Chunk.WIDTH));
	}
	...
	public Block getBlock(Vector3 pos) {
		if (inBounds(pos)) {
			return this.getChunk(pos).blocks[posMod((int) Math.floor(pos.x), Chunk.WIDTH)][posMod((int) Math.floor(pos.z), Chunk.WIDTH)][(int) Math.floor(pos.y)];
		} else {
	...
	public void setBlock(Block block) {
		if (inBounds(block.a) && inBounds(block.b) && block.isOnGrid()) {
			this.getChunk(block.a).blocks[posMod((int) block.a.x, Chunk.WIDTH)][posMod((int) block.a.z, Chunk.WIDTH)][(int) block.a.y] = block;
	...
	public boolean inBounds(Vector3 pos) {
		return (pos.y <= (Chunk.HEIGHT - 1) && pos.y >= 0f);
	}
}
