...
// Standard Phong
 ...
 //    vec4 specular =vec4( pow(max(dot(R, E), 0.0),exponent));
	...
	vec4 semifinal = (refr+ refl*(1.0-fresnelTerm))+specular;

	vec3 mycolor = vec3(0.2, 0.5, 0.7);
	vec3 finalcolor = (semifinal.rgb + mycolor.rgb)/2;
	
	
	float myTransparency = 0.6;
    gl_FragColor = vec4(finalcolor.rgb, myTransparency );
